...
When you set up the gateway, add the key to the environment file loaded by the gateway. On **gateway v1.2 and above**, this is `api-platform.env` (generated by `./scripts/setup.sh`); on earlier versions it is `configs/keys.env`:
