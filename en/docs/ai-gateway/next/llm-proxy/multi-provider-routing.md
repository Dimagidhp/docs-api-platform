...
Multi-provider routing lets one LLM proxy expose a single OpenAI-compatible endpoint while routing each request to a selected LLM provider. Applications keep one endpoint and one request and response format, even when the upstream provider changes.
